...
class UsageTracker:
    ...
    def _extract_context(self, file_path: str, line_number: int, context_lines: int = 3) -> str:
        """
    ...
    def get_usage_patterns(self, symbol_id: int) -> List[str]:
        ...
        if usages:
            ...
            if count / len(usages) > 0.7:
                patterns.append(f"File-local utility (70%+ usage in {most_common_file})")
        
        # Pattern: Called from many different files (shared utility)
        unique_files = len(set(u['file'] for u in usages))
        if unique_files > 5:
        ...
        if caller_counter:
            ...
            if count / len(usages) > 0.6:
                patterns.append(f"Helper function (60%+ calls from {most_common_caller})")
        
        # Pattern: Import-only (module/class)
        import_ratio = sum(1 for u in usages if u['reference_type'] == 'IMPORTS') / max(len(usages), 1)
        if import_ratio > 0.8:
    ...
    def get_usage_examples(self, symbol_id: int, limit: int = 5) -> List[Dict]:
        ...
        for usage in usages:
            if usage['file'] not in seen_files or len(examples) < limit:
                if usage['context']:
                    examples.append({
                        'file': usage['file'],
                        'line': usage['line'],
    ...
    def find_unused_symbols(self, file_id: Optional[int] = None) -> List[Dict]:
        """
        ...
        for symbol in symbols:
            ...
            ref_count = self.session.exec(
                select(Reference)
                .where(Reference.target_symbol_id == symbol.id)
            ).all()
