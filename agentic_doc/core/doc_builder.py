...
from agentic_doc.llm.providers import get_provider
from agentic_doc.llm.core import LLMClient
from agentic_doc.core.prompts import (
    FILE_DOC_SYSTEM_PROMPT, FILE_DOC_USER_PROMPT,
    USE_CASE_DOC_SYSTEM_PROMPT, USE_CASE_DOC_USER_PROMPT,
    FUNCTION_USAGE_DOC_PROMPT, DEPENDENCY_DOC_PROMPT
)
...
"""
    ...
    def generate_dir_docs(self):
        ...
        for dir_path in track(sorted(directories), description="Generating dir docs..."):
    ...
    def _process_directory(self, dir_path: Path, rel_path: Path):
        ...
        for file in dir_path.glob("*.md"):
            ...
            file_name = file.stem
            file_summaries.append(f"### [{file_name}](../files/{rel_path}/{file_name}.md)\n{content[:300]}...")
...
Reference specific functions and classes from the provided index."""
    ...
    def generate_arch_doc(self):
        ...
        symbol_types = session.exec(
            select(Symbol.kind, sa_func.count(Symbol.id))
...
Create a clear, insightful architecture overview that helps developers understand the system structure."""
    ...
    def generate_use_case_docs(self, min_usage: int = 5, limit: int = 20):
        ...
        for symbol_info in track(hot_symbols, description="Generating use cases..."):
            try:
                ...
                console.print(f"[red]Failed to generate use case for {symbol_info['name']}: {e}[/red]")
        
        console.print("[green]Use case documentation complete.[/green]")
        session.close()
    
    def _generate_use_case(self, session, symbol_info: dict, usage_tracker: UsageTracker):
    ...
    def generate_function_usage_docs(self, symbol_name: str = None):
        ...
        console.print("[bold blue]Generating function usage documentation...[/bold blue]")
        ...
        if not symbols:
            console.print(f"[yellow]No symbols found.[/yellow]")
            session.close()
            return
        ...
        for symbol in track(symbols, description="Generating usage docs..."):
            try:
                self._generate_function_usage(session, symbol, usage_tracker, dep_analyzer)
            except Exception as e:
                console.print(f"[red]Failed to generate usage doc for {symbol.name}: {e}[/red]")
        
        console.print("[green]Function usage documentation complete.[/green]")
        session.close()
    
    def _generate_function_usage(self, session, symbol: Symbol, usage_tracker: UsageTracker, dep_analyzer: DependencyAnalyzer):
        ...
        prompt = FUNCTION_USAGE_DOC_PROMPT.format(
            ...
            common_callers=', '.join(stats['most_common_callers']) if stats['most_common_callers'] else 'None',
            patterns='\n- '.join(patterns) if patterns else 'No patterns detected'
        )
        
        doc_content = self.llm.generate(prompt, "You are documenting function usage patterns.")
