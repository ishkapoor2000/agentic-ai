...
from agentic_doc.db.schema import Symbol, File
...
class DependencyVisualizer:
    ...
    def generate_file_dependency_mermaid(self, scope_files: Optional[List[str]] = None) -> str:
        """
        ...
        for func in hot_functions[:5]:
            ...
            if symbol:
                ...
                for dependent in dependents[:5]:
                    lines.append(f"- `{dependent['source_name']}` ({dependent['reference_type']})")
                
                lines.append("")
        
        return "\\n".join(lines)
    
    def generate_usage_heatmap_data(self, file_id: Optional[int] = None) -> Dict:
        """
        ...
        return {
            ...
            'metadata': {
                ...
                'total_files': len(set(
                    file for item in heatmap_data 
                    for file in item['usage_by_file'].keys()
                ))
            }
        }
