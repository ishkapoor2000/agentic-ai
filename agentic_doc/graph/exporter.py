...
class GraphExporter:
    ...
    def export_dot(self) -> str:
        ...
        for file in files:
            ...
            dot_lines.append(f'  file_{file.id} [label="{label}", style=filled, fillcolor=lightblue];')
        ...
        for symbol in symbols:
            ...
            dot_lines.append(f'  symbol_{symbol.id} [label="{label}", style=filled, fillcolor={color}];')
        ...
        for dir_name, dir_file_list in sorted(dir_files.items()):
            ...
            for file in dir_file_list:
                ...
                symbols = self.session.exec(
                    select(Symbol)
                    .where(Symbol.file_id == file.id)
                    .limit(5)
                ).all()
                
                for symbol in symbols:
                    symbol_id = f"sym_{symbol.id}"
                    mermaid_lines.append(f"    {symbol_id}[{symbol.kind}: {symbol.name}]")
    ...
    def export_html(self) -> str:
        ...
        html = html.replace('</body>', f'{mermaid_script}\n{data_script}\n{js_content}\n</body>')
